...
range = map (uncurry $ \c => maybe (Single c) (Interval c))
            (maybeTOK isCHAR `andopt` (box $ exactTOK DOTS `rand` box (maybeTOK isCHAR)))

regexp : All (Parser' RegExp)
regexp = fix _ $ \rec =>
           let parens    = between (exactTOK LPAR) (box $ exactTOK RPAR)
               parensopt = betweenopt (exactTOK LPAR) (box $ exactTOK RPAR)
               ranges    = Combinators.app ((cmap Bracket (exactTOK OPEN)) `alt` (cmap (BracketNot . NEList.toList) (exactTOK NOPEN)))
                                           (box $ (nelist range) `land` (box $ exact CLOSE))
               ...
               star      = map (\(r,m) => maybe r (const $ Star r) m) (base `andopt` box (exact STAR))
               disj      = chainr1 star (box $ Disj `cmap` exactTOK OR)
              in
