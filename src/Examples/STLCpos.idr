...
cut rec = map (\(v,p,t) => Cut p v t) $
  ...
  where
    ...
    andbox p q =
      Nat.map2 {a=Parser' _} {b=Parser' _} (\p, q => Combinators.and p (box q)) p (box q)
...
neu recv recn =
  hchainl
    ...
    (box $ map STLCpos.App $ randm space getPosition)
    (box $ STLCpos.app recv)

lam : All (Box (Parser' Val) :-> Parser' Val)
lam rec = map (\((p,s),v) => Lam p s v) $
          rand (char '\\')
               (box $ and (mand getPosition
                               (withSpaces ident))
